...
class CMB_Select extends CMB_Field {
	...
	public function enqueue_scripts() {
        ...
        if(!in_array($screen->id, array('product', 'shop_coupon', 'shop_order'))){
			wp_enqueue_script( 'cmb-select2', trailingslashit( CMB_URL ) . 'js/vendor/select2/select2.js', array( 'jquery' ) );
			wp_enqueue_script( 'cmb-field-select', trailingslashit( CMB_URL ) . 'js/field.select.js', array( 'jquery', 'select2', 'cmb-scripts' ) );
		}
	}
