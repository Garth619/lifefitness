...
add_action( 'after_setup_theme', 'remove_wti_ct' );	
function remove_wti_ct(){
...
function tm_video($post_id, $autoplay = false) {
	...
	elseif(!empty($url)) {
		...
		// WordPress Embeds
			...
			if ( preg_match('/vimeo\.com/i', $url ) && $autoplay ) {
				$video = preg_replace('/<iframe/', '<iframe allow="autoplay"', $video);
			}
			if(trim($video) == $url) {
	...
	if($arr){
		...
		foreach($arr as $group){ ?>
			<div class="multilink-item">
                <span class="mtitle"><?php echo isset($group['title'])?$group['title']:'' ?></span>
                <span class="mlink">
                <?php 
				...
				foreach($multi_link as $link){
					if(strpos($link, 'http') !== false){ //is a url
						...
						?>
                        <a class="multilink-btn btn btn-sm btn-default bordercolor2hover bgcolor2hover <?php if(isset($_GET['link']) && $_GET['link']==$link_count){ echo 'current-link'; } ?>" href="<?php echo add_query_arg( 'link', $link_count, get_permalink(get_the_ID()) ); ?>"><i class="fa fa-angle-right"></i> <?php echo $temp_title?$temp_title:esc_html__('Link ','cactus').($link_number+1); ?> </a>
                        <?php
						$temp_title = '';
						$link_count++;
						$link_number++;
