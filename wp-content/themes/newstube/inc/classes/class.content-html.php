		...
		function get_item_video($thumb, $show_title, $show_rate, $show_dur, $themes_pur, $quick_view='def'){	
		  ...
		  $html .= '
				<div class="qv_tooltip"  title="
					...
					<div class= \'gv-button\'>';
						if($format=='video'){
							$html .= '<div class=\'quick-view\'><a href='.get_permalink().' title=\''.esc_attr(get_the_title()).'\'>'.esc_html__('Watch Now','cactus').'</a></div>';
						}else{
							$html .= '<div class=\'quick-view\'><a href='.get_permalink().' title=\''.esc_attr(get_the_title()).'\'>'.esc_html__('Read more','cactus').'</a></div>';
						}
						$html .='
		  ...
		  $quick_if = $quick_view=='def'?ot_get_option('quick_view_info'):$quick_view;
			...
			$format = get_post_format(get_the_ID());
			  ...
			  $html .= '
				...
				$category_str 	= cactus_get_category($categories);
				$html .= $category_str . tm_post_rating(get_the_ID());
				  $html .= '
				  </div>';
					if($quick_if=='1'){
						$html .='</div>';
					...
					<div class="video-item">
						<div class="videos-row">
							...
							<div class="item-info">
							  ...
							  <div class="item-meta">';
							  if($show_view!='hide_v'){
									$widget_data .= '<span class="pp-icon"><i class="fa fa-eye"></i> '.cactus_get_post_view($post->post_id).'</span><br>';
							  }
